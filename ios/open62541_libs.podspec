Pod::Spec.new do |s|
  ...
  s.prepare_command = <<-CMD
    ...
    cmake -B build -DUA_LOGLEVEL=100 -DBUILD_SHARED_LIBS=OFF -DUA_ENABLE_INLINABLE_EXPORT=ON -DCMAKE_BUILD_TYPE=Release -DUA_ENABLE_ENCRYPTION=MBEDTLS -DUA_MULTITHREADING=0 -DMBEDTLS_INCLUDE_DIRS=$MBEDTLS_INCLUDE_DIRS -DMBEDTLS_LIBRARY=$MBEDTLS_LIBRARY -DMBEDX509_LIBRARY=$MBEDX509_LIBRARY -DMBEDCRYPTO_LIBRARY=$MBEDCRYPTO_LIBRARY -DCMAKE_OSX_SYSROOT=iphoneos -DCMAKE_OSX_ARCHITECTURES=arm64 -DCMAKE_OSX_DEPLOYMENT_TARGET=12.0
    cmake --build build
